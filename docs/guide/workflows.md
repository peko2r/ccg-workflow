...
拿到任务
  │
  ├─ 很简单，一句话说清？ ──→ /ccx:frontend 或 /ccx:backend
  │
  ├─ 想先看看计划？ ────────→ /ccx:plan → /ccx:execute
  │
  ├─ 不想让 AI 乱来？ ─────→ /ccx:spec-* 系列
  │
  ├─ 能拆成 3+ 个模块？ ───→ /ccx:team-* 系列
  │
  └─ 从头到尾全包？ ────────→ /ccx:workflow
```
...
# 两种执行方式，选一个：
/ccx:execute .claude/plan/user-auth.md   # Claude 亲自干，精细控制
/ccx:codex-exec .claude/plan/user-auth.md  # Codex 全干，Claude 只审核
```
...
`/ccx:workflow` 自动跑完 6 个阶段：研究→构思→计划→执行→优化→评审。

```bash
/ccx:workflow 实现完整的用户认证，注册、登录、JWT
```
